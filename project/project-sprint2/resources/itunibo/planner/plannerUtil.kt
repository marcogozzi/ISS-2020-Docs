 ...
 */
    ...
    @JvmStatic fun setGoal( x: Int, y: Int) {
        try {
             if(debug)println("setGoal $x,$y while robot in cell: ${getPosX()},${getPosY()} direction=${getDirection()} ") //canMove=$canMove
    ...
    @JvmStatic fun doPlan(): List<Action>? {
		...
		if(debug)println("plannerUtil doPlan actions=$actions")
        ...
        if (actions == null || actions!!.isEmpty()) {
            ...
            if(debug)println("plannerUtil doPlan NoOp")
            return null
        }
...
*/	
	...
	@JvmStatic fun getMapHTML() : String{
		var table = RoomMap.getRoomMap().toHtmlTable()
		if(robotState!!.getDirection() == RobotState.Direction.UP) 
			table = table.replace(Regex("class=\"t.\""),"class=\"robot\"")
						.replace(Regex("class=\"b.\""),"class=\"robotfree\"")
		if(robotState!!.getDirection() == RobotState.Direction.RIGHT) 
			table = table.replace(Regex("class=\".r\""),"class=\"robot\"")
						.replace(Regex("class=\".l\""),"class=\"robotfree\"")
		if(robotState!!.getDirection() == RobotState.Direction.DOWN) 
			table = table.replace(Regex("class=\"b.\""),"class=\"robot\"")
						.replace(Regex("class=\"t.\""),"class=\"robotfree\"")
		if(robotState!!.getDirection() == RobotState.Direction.LEFT) 
			table = table.replace(Regex("class=\".l\""),"class=\"robot\"")
						.replace(Regex("class=\".r\""),"class=\"robotfree\"")
		return table
	}
