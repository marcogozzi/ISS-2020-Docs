...
//			Thread.sleep(3000)
	...
	fun testStopCleaning(){
		...
		runBlocking{
			...
			while( (waiter.geResourceRep().contains("gocleanteatable"))
					.or(waiter.geResourceRep().contains("checkAvailability"))
					.or(waiter.geResourceRep().contains("cancelplan"))
					.or(waiter.geResourceRep().contains("reach")))
				delay(50)
			
			
			assertTrue(waiter.geResourceRep().contains("entrancedoor")
					.or(waiter.geResourceRep().contains("waiter(teatable")))
			
			println(" --- testStopCleaning ends ---")	
		}
	}
	/*
